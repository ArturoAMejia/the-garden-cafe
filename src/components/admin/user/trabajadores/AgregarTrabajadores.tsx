...
import { ICargo, IEstadoCivil, IGrupoUsuario } from "../../../../interfaces";
...
const AgregarTrabajadores: FC<Props> = ({ estado_civil, grupos }) => {
  ...
  const { registrarTrabajador } = useContext(AdminContext);
  const onAgregarTrabajadores = async ({
    cedula_ruc,
    nombre,
    ...
    const { hasError, message } = await registrarTrabajador(
      cedula_ruc,
      nombre,
      apellido_razon_social,
      fecha_nacimiento_constitucion,
              ...
              >
                <Dialog.Panel className="h-auto w-full max-w-5xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <form
                    ...
                    onSubmit={handleSubmit(onAgregarTrabajadores)}
                  >
